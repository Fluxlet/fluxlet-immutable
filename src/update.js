...
export function update(path, value) {
    ...
    return (root) => {
        ...
        function doUpdate(container, step) {
            // check we have a valid container
            if (!container || !(typeof container === "object")) {
                throw new TypeError(`update of path '${path.join('.')}' failed, expecting an Object or Array at: '${path.slice(0, step)}' got: ${container}`)
            }
            ...
            var value = step + 1 < path.length ? doUpdate(container[key], step + 1) : getValue(container[key], root)
